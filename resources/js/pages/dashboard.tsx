...
function DeltaBadge({ diff, suffix = '' }: { diff: number; suffix?: string }) {
    return (
        <span className={`text-xs font-bold ${diffColor(diff)}`}>
            {diffArrow(diff)} {diff > 0 ? '+' : ''}{diff}{suffix}
        </span>
    );
}
...
export default function Dashboard({
    ...
    const insights = useMemo(
        () => hasTwoWeeks ? generateInsights(comparison, exerciseBreakdown, currentWeek, previousWeek) : [],
        [comparison, exerciseBreakdown, currentWeek, previousWeek, hasTwoWeeks]
    );

    // Trend from last 2 data points
    const trend = useMemo(() => {
    ...
    const statusCfg = statusConfig(comparison.status);
    ...
    return (
        <>
            ...
            <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 p-4 md:p-8 pb-24">
                ...
                {/* ── Empty State ── */}
                {!hasData && (
                    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-800 bg-neutral-900/20 py-24 gap-5 text-center">
                        ...
                        <div className="space-y-2 max-w-xs">
                            <h2 className="text-xl font-black text-neutral-100">No data yet</h2>
                            <p className="text-sm text-neutral-500 leading-relaxed">
                                Complete your first workout session to start seeing weekly performance analytics here.
                            </p>
                        </div>
                    </div>
                )}
                    ...
                    <>
                            ...
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div className="space-y-0.5">
                                    ...
                                    <p className="text-xs text-neutral-500">
                                        ...
                                        {trend === 'neutral' && <span className="text-neutral-500">Building your history...</span>}
                        ...
                        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                            ...
                            <SummaryCard
                                ...
                                diff={parseFloat((comparison.volumeDiff / Math.max(1, Math.abs(comparison.volumeDiff) < 1000 ? 1 : 1000)).toFixed(1))}
                                diffSuffix={Math.abs(comparison.volumeDiff) >= 1000 ? 'k kg' : ' kg'}
                        ...
                        <div className="rounded-2xl border border-neutral-800 bg-neutral-900/60 p-5">
                            <h2 className="text-sm font-bold uppercase tracking-widest text-neutral-500 mb-4">This Week</h2>
                            ...
                            <div className={`rounded-2xl border p-5 space-y-5 ${statusCfg.bg}`}>
                                <div className="flex items-center justify-between flex-wrap gap-3">
                                    <div className="flex items-center gap-3">
                                        <span className={`text-2xl font-black ${statusCfg.color}`}>
                                ...
                                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                                    {[
                                        { label: 'Volume Diff',  value: `${comparison.volumeDiff >= 0 ? '+' : ''}${fmtVol(comparison.volumeDiff)} kg`, diff: comparison.volumeDiff },
                                        { label: 'Avg Weight Δ', value: `${comparison.avgWeightDiff >= 0 ? '+' : ''}${fmt(comparison.avgWeightDiff)} kg`, diff: comparison.avgWeightDiff },
                                        { label: 'Avg Reps Δ',   value: `${comparison.avgRepsDiff >= 0 ? '+' : ''}${fmt(comparison.avgRepsDiff)}`, diff: comparison.avgRepsDiff },
                                        { label: 'Workouts Δ',   value: `${comparison.frequencyDiff >= 0 ? '+' : ''}${comparison.frequencyDiff}`, diff: comparison.frequencyDiff },
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-neutral-500">{exerciseBreakdown.length} exercises compared week-over-week</p>
                                        </div>
                                    </div>
                                    {breakdownOpen
                                            ...
                                            <table className="w-full text-sm">
                                                <thead>
                                                    <tr className="border-b border-neutral-800 text-[10px] font-black uppercase tracking-widest text-neutral-600">
                                                        ...
                                                        <th className="px-4 py-3 text-right">Prev Week</th>
                                                        <th className="px-4 py-3 text-right">Curr Week</th>
                        ...
                        {!hasTwoWeeks && (
                            <div className="rounded-2xl border border-dashed border-neutral-800 bg-neutral-900/20 py-10 text-center space-y-2">
                                ...
                                <p className="text-sm font-bold text-neutral-400">Complete workouts across two weeks to unlock comparisons</p>
                                <p className="text-xs text-neutral-600">Week-over-week progress, exercise breakdown, and insights will appear here.</p>
                            </div>
                        )}
                    </>
                )}
